...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        # Calculate the number of training samples which are currently used.
        total_train = 0
        for user in self.selected_users:
            total_train += user.train_samples
        
        # Calculate the weighted sum of local parameters.
        for user in self.selected_users:
    ...
    def select_users(self, round, num_users, return_idx=False):
        ...
        '''
        # If num_users equals the number of total users, simply returns all users
        if(num_users == len(self.users)):
            print("All users are selected")
            return self.users

        # Otherwise, randomly choose {num_users} users from all users.
        # Taking min{num_users, len(self.users)} to ensure that num_users is not larger
        # than the total number of users.
        num_users = min(num_users, len(self.users))
        print("Select %d users" % (num_users))
    ...
    def plot(self):
      ...
      # Check if 'glob_loss' is contained in self.metrics
      if 'glob_loss' not in self.metrics.keys():
        print("Unable to plot loss")
      else:
        plt.figure('Loss')
        plt.plot(self.metrics['glob_loss'], label='Loss per iter')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.ylim([0, 3])
        ...
        # Save plot
        fig_path = os.path.join(self.save_path, self.algorithm, "models", self.dataset)
        if not os.path.exists(fig_path):
            os.makedirs(fig_path)
        plt.savefig(os.path.join(fig_path, "loss.png"))

      # Check if 'glob_acc' is contained in self.metrics
      if 'glob_acc' not in self.metrics.keys():
        print("Unable to plot accuracy")
      else:      
        plt.figure('Accuracy')
        plt.plot(self.metrics['glob_acc'], label='Accuracy per iter')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.legend(loc='lower right')
        ...
        if not os.path.exists(fig_path):
            os.makedirs(fig_path)
        plt.savefig(os.path.join(fig_path, "accuracy.png"))
